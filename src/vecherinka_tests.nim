...
when defined(vecherinka_ir_tests):
  ...
  let stringify = so(int, string, input): pure($input)
  ...
  let variant_step = so(Message, Message, input): pure(input)
  let variant_lift = lift(Message(kind: MessageKind, text: here))[variant_step]
  ...
  let second_to_record = so(Second, LiftRecord, input): pure(LiftRecord())
  ...
  assert_lift_shape(LiftRecord(name: string, count: int), "lpk_object", 0)
  assert_lift_shape(Message(kind: MessageKind, text: here), "lpk_object", 1)
  assert_lift_shape(Message(kind: MessageKind, text: string),
    "lpk_object", 0)
  ...
  static:
    ...
    doAssert compiles(parse_lift_only(Message(kind: MessageKind, text: here)))
    doAssert not compiles(parse_lift_only(LiftRecord(name: "literal", count: int)))
